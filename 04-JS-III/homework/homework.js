...
function dePalabrasAFrase(palabras) {
  ...
  //palabras.join(" ")  otra forma de hacer mas facil
}
...
function cuentoElementos(arreglo){
  ...
  for(let i = 0; i < arreglo.length; i++){
    if(arreglo[i] > 18) mayor.push(arreglo[i])
  } return mayor.length;
...
function mesesDelAño(array) {
  ...
  let mesesNuevos = [];
  for (let i = 0; i < array.length; i++) {
    if(array[i] === "Enero" || array[i] === "Marzo" || array[i] === "Noviembre") mesesNuevos.push(array[i]);
  }
  if(mesesNuevos.length === 3) return mesesNuevos;
